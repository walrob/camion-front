    ...
    <template v-slot:item.actions="{ item }">
      ...
      <v-tooltip v-if="item.isActive" location="top" text="Desactivar">
        <template v-slot:activator="{ props }">
          ...
          >
            <PhotoXIcon size="20" stroke-width="1.5" />
          </v-btn>
        </template>
      </v-tooltip>
      <v-tooltip v-else location="top" text="Activar">
        <template v-slot:activator="{ props }">
          ...
          >
            <PhotoCheckIcon size="20" stroke-width="1.5" />
          </v-btn>
        </template>
      </v-tooltip>
    </template>
...
import { EditIcon, PhotoCheckIcon, PhotoXIcon } from "vue-tabler-icons";
...
const bannerData = ref<Banner | undefined>(undefined);

const enableItem = (item: Banner) => {
  bannerData.value = item;
  textConfirm.value = `<p>Se habilitará el banner <b>${item.name}</b></p><p>¿Está seguro de realizar la operación?</p>`;
  titleConfirm.value = "Habilitar banner";
  showConfirm.value = true;
};

const blockItem = (item: Banner) => {
  bannerData.value = item;
  textConfirm.value = `<p>Se dehabilitará el banner <b>${item.name}</b></p><p>¿Está seguro de realizar la operación?</p>`;
  titleConfirm.value = "Bloquear banner";
  showConfirm.value = true;
};

const confirmSave = (value: ResponseConfirm) => {
  if (value.resp) {
    emit("change-active", bannerData.value);
